...
public record SAPConnectorRequest(
    ...
    @TemplateProperty(group = "sap",
        ...
        feel = FeelMode.optional) @NotEmpty String destination,

    @TemplateProperty(group = "sap",
        label = "OData base service path",
        description = "absolute base path, e.g. /sap/opu/odata/dmo/case",
        feel = FeelMode.optional) @Pattern(regexp = "^(\\|=).*",
        message = "oDataService must start with a '/'") @NotEmpty String oDataService,
    ...
    @TemplateProperty(group = "sap",
        ...
        feel = FeelMode.optional) @Pattern(regexp = "^[^/].*$",
        message = "entityOrEntitySet must not start with a '/'") @NotEmpty String entityOrEntitySet,
    @Valid HttpMethod httpMethod) {
  @TemplateDiscriminatorProperty(group = "sap",
      label = "OData version",
  ...
  @TemplateDiscriminatorProperty(name = "httpMethod",
      label = "Http method",
  ...
  public sealed interface HttpMethod {
    ...
    record Get(@TemplateProperty(group = "advanced",
        label = "$filter",
        description = "$filter on EntitySet",
        optional = true) String filter,

               @TemplateProperty(group = "advanced",
                   label = "$top",
                   description = "only the first $top results of an EntitySet",
                   optional = true) Long top,

               @TemplateProperty(group = "advanced",
                   label = "$skip",
                   description = "skip the first $skip results of an EntitySet",
                   optional = true) Long skip,

               @TemplateProperty(group = "advanced",
                   label = "$orderBy",
                   description = "order the EntitySet by one or more properties",
                   optional = true) String orderBy,

               @TemplateProperty(group = "advanced",
                   label = "$expand",
                   description = "expand an Entity/-Set by another Entity/-Set",
                   optional = true) String expand,

               @TemplateProperty(group = "select",
                   label = "$select",
                   description = "only select $select properties of an Entity/-Set",
                   optional = true) String select,

               @TemplateProperty(group = "advanced",
                   label = "$count",
                   description = "$count of EntitySet",
                   optional = true) Boolean count,

               @Valid ODataVersionGet oDataVersionGet) implements HttpMethod {

      @JsonTypeInfo(use = Id.NAME, property = "oDataVersionGet")
      @JsonSubTypes({
          @Type(value = V2.class, name = "V2"), @Type(value = V4.class, name = "V4")
      })
      @TemplateDiscriminatorProperty(group = "sap",
          label = "OData version",
          description = "what version of the OData protocol the above service uses",
      ...
      public sealed interface ODataVersionGet {
        ...
        record V4(
            ...
            @TemplateProperty(group = "advanced",
                ...
                optional = true) Boolean inlinecount,

            @TemplateProperty(group = "advanced",
                label = "$search",
                description = "search for $search in EntitySet",
                optional = true) String search) implements ODataVersionGet {
    ...
    record Post(
        ...
        @TemplateProperty(group = "sap",
            label = "OData version",
            description = "what version of the OData protocol the above service uses") ODataVersion oDataVersionPost,

        @TemplateProperty(group = "sap",
            label = "Request body",
            defaultValue = "={}") @NotEmpty Map<String, Object> payloadPost) implements HttpMethod {}

    @TemplateSubType(id = "put", label = "Put")
    record Put(@TemplateProperty(group = "sap",
        label = "OData version",
        description = "what version of the OData protocol the above service uses") ODataVersion oDataVersionPut,

               @TemplateProperty(group = "sap",
                   label = "Request body",
                   defaultValue = "={}") @NotEmpty Map<String, Object> payloadPut) implements HttpMethod {}
    ...
    record Patch(
        ...
        @TemplateProperty(group = "sap",
            label = "OData version",
            description = "what version of the OData protocol the above service uses") ODataVersion oDataVersionPatch,

        @TemplateProperty(group = "sap",
            label = "Request body",
            defaultValue = "={}") @NotEmpty Map<String, Object> payloadPatch) implements HttpMethod {}

    @TemplateSubType(id = "delete", label = "Delete")
    record Delete(@TemplateProperty(group = "sap",
        label = "OData version",
        description = "what version of the OData protocol the above service uses") ODataVersion oDataVersionDelete)
