...
public class DestinationProviderTest {
  ...
  public void success() {
    ...
    var destination =
        DefaultHttpDestination.builder("http://localhost:4004").property("name", "httpDestination").build();
    DestinationAccessor.prependDestinationLoader((name, options) -> Try.success(destination));

    var resolvedDestination = DestinationProvider.getDestination("httpDestination", DestinationType.HTTP);
  ...
  public void retry_then_success() {
    ...
    try (MockedStatic<DestinationAccessor> mockedDestinationAccessor = mockStatic(DestinationAccessor.class)) {
      mockedDestinationAccessor.when(() -> DestinationAccessor.getDestination(destinationName))
      ...
      mockedDestinationAccessor.verify(() -> DestinationAccessor.getDestination(destinationName), times(4));
    }
  }
  ...
  public void retry_then_fail() {
    try (MockedStatic<DestinationAccessor> mockedDestinationAccessor = mockStatic(DestinationAccessor.class)) {
      mockedDestinationAccessor.when(() -> DestinationAccessor.getDestination("invalidDestination"))
          .thenThrow(new RuntimeException("Destination not found"));
      ...
      mockedDestinationAccessor.verify(() -> DestinationAccessor.getDestination("invalidDestination"), times(10));
    }
  }
}